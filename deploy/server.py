...
#Abre o modelo salvo no Pickle
o_modelo = pickle.load(open('modelo.pkl', 'rb'))

#Define a rota de predição da API
@app.route('/predict', methods=['GET', 'POST'])

# Função que recebe os sintomas do POST do json e retorna a condição médica predita pelo modelo
def predict():
    #Checa se o método do json é post
    if request.method == "POST":
        #Pega os dados do post
        data = request.get_json()
        #Define os sintomas que estão disponíveis no modelo
        features = ['itching', 'skin_rash', 'nodal_skin_eruptions', 'continuous_sneezing',
                    ...
                    'redness_of_eyes', 'sinus_pressure', 'runny_nose',
        ...
        #Cria um dicionário com cada sintoma como key e 0 para todos os values
        feature_dict = {feature: 0 for feature in features}
        
        # Coloca 1 para cada sintoma recebido pelo post do json
        for i in feature_dict.keys():
            if i in data.keys() and data[i]:
                feature_dict[i] = 1
            else:
                pass
        #Prediz a condição médica
        df = pd.DataFrame(feature_dict, index=[0])
        pred = np.array(o_modelo.predict(df)).tolist()

        #Retorna a condição médica predita
        return {'condicao_medica_predita': pred[0]}
